        ...
        {navItems.map((item, idx) => {
          if (item.isPrimary) {
            return (
              <motion.button 
                ...
                whileTap={{ scale: 0.98 }}
                className="mt-4 flex items-center justify-center gap-2 bg-[#0052FF] text-white p-3 rounded-xl font-medium shadow-lg shadow-blue-500/20"
              >
          ...
          return (
            <button 
              ...
              className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isActive ? 'bg-blue-50 text-[#0052FF] font-semibold' : 'text-slate-500 hover:bg-slate-50'}`}
            >
              {item.icon}
      ...
      {navItems.map((item, idx) => {
        if (item.isPrimary) {
          return (
            <motion.button
              ...
              onClick={() => navigate('group-action', { goBackTo: currentRoute })}
              whileTap={{ scale: 0.9 }}
              className="bg-[#0052FF] p-4 rounded-full shadow-lg shadow-blue-500/30 transform -translate-y-4 text-white"
            >
              <Plus size={24} />
            </motion.button>
          )
        ...
        return (
          <button 
            key={idx}
            onClick={() => navigate(item.id as any)}
            className={`flex flex-col items-center gap-1 ${isActive ? 'text-[#0052FF]' : 'text-slate-400'}`}
          >
            {item.icon}
